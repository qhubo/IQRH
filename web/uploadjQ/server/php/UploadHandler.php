...
class UploadHandler {
    ...
    function __construct($idcarpeta, $ruta, $url, $servidor, $usuario, $clave, $baseDatos, $options = null, $initialize = true, $error_messages = null) {
        $idcarpeta = str_replace("/", "", $idcarpeta);

//        $fp = fopen("logu.txt", "w");
//        fputs($fp, serialize($ruta));
//        fclose($fp);
        $fp = fopen("logc.txt", "w");
        fputs($fp, serialize($ruta));
        fclose($fp);
        $carpetaid = (string) $idcarpeta;
        $this->response = array();
        $this->options = array(
            ...
            'readfile_chunk_size' => 10 * 1024 * 1024 * 35000, // 10 MiB
...
//            echo    $this->options['script_url'].$this->get_query_separator($this->options['script_url']);
//               die(); 
        if ($options) {
        ...
        if ($initialize) {
            $this->initialize($carpetaid, $servidor, $usuario, $clave, $baseDatos,$url);
        }
    }

    protected function initialize($carpetaid, $servidor, $usuario, $clave, $baseDatos,$url) {
        $carpetaid = (string) $carpetaid;
        $carpetaid = trim($carpetaid);
        $carpetaid = str_replace("/", "", $carpetaid);
        $fp = fopen("log.txt", "w");
        fputs($fp, serialize($usuario.",".$clave.", ".$baseDatos));
        fclose($fp);
        ...
        switch ($this->get_server_var('REQUEST_METHOD')) {
            ...
            case 'PUT':
            case 'POST':
                $mbd2 = new PDO('mysql:host=' . $servidor . ';dbname=' . $baseDatos, $usuario, $clave);
                $sql = "select  count(*) as existe from  multimedia where identificador = '" . $carpetaid . "'";
                foreach ($mbd2->query($sql) as $filaSym) {
                    $existe = $filaSym['existe'];
                }
                if ($existe == 0) {
                    $sql = "insert into multimedia(nombre,identificador,cargado) values('','" . $carpetaid . "',0)";
                } else {
                    $sql = "update multimedia set cargado=0  where identificador = '" . $carpetaid . "'";
                }
                $mbd2->query($sql);
                   $fp = fopen("logins.txt", "w");
        fputs($fp, serialize($usuario.",--".$carpetaid."--".$clave.", ".$baseDatos));
        fclose($fp);
        
                $this->post($this->options['print_response'], $carpetaid);
                $areglo = explode("uploads",$url);
                $urlRest= $areglo[0]."index.php/rest/carga";
               // $urlRest='http://sistema.crackchapin.com/index.php/rest/carga';
                $fp = fopen("logu.txt", "w");
                fputs($fp, serialize($urlRest));
                fclose($fp);                
                $handler = curl_init();
                curl_setopt($handler, CURLOPT_RETURNTRANSFER, TRUE);
                curl_setopt($handler, CURLOPT_URL, $urlRest);
            ...
            case 'DELETE':
                ...
                fclose($fp);
                $mbd2X->query($sql);
                $this->delete($this->options['print_response'], $carpetaid);
                $areglo = explode("uploads",$url);
                $urlRest= $areglo[0]."index.php/rest/carga"; 
             //  $urlRest='http://sistema.crackchapin.com/index.php/rest/carga';
                $fp = fopen("logu.txt", "w");
...
//              $fp = fopen("log.txt", "w");
//              // fputs($fp, serialize($this->get_file_name_param()));
//                fputs($fp, serialize(  $this->options['upload_dir'].$this->get_user_path().""
//            .$version_path. "---".$file_name));
//               fclose($fp);
        $idcarpeta = $this->options['carpeta_id'];
        $retorno = $this->options['upload_dir'] . $this->get_user_path()
                . $version_path . $file_name;
        $retorno = str_replace("//", "/" . $idcarpeta . "/", $retorno);
        $fp = fopen("log.txt", "w");
        fputs($fp, serialize($this->options));
        fclose($fp);
        return $retorno;
    }
...
//           echo $file->deleteUrl;
//           echo "<hr>";
//        $file->deleteUrl = $this->options['upload_url'].'index.php?'.$this->get_singular_param_name()
//            .'='.rawurlencode($file->name);
//           
//        echo $file->deleteUrl;
//        die();
//        
    ...
    public function post($print_response = true, $carpetaid) {
        if ($this->get_query_param('_method') === 'DELETE') {
            $idcarpeta = $this->options['carpeta_id'];
            return $this->delete($print_response, $idcarpeta);
        }
    ...
    public function delete($print_response = true, $idcarpeta) {
        ...
        foreach ($file_names as $file_name) {
            $file_path = $this->get_upload_path($file_name);
            $file_path = str_replace("//", "/" . $idcarpeta . "/", $file_path);  //
//                $fp = fopen("log.txt", "w");
//              // fputs($fp, serialize($this->get_file_name_param()));
//                fputs($fp, serialize( $file_path));
//               fclose($fp);
//               

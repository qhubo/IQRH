 ...
 * @method ProyeccionVacacion findOneByDiaVacacion(boolean $dia_vacacion) Return the first ProyeccionVacacion filtered by the dia_vacacion column
 ...
 * @method array findByDiaVacacion(boolean $dia_vacacion) Return ProyeccionVacacion objects filtered by the dia_vacacion column
...
abstract class BaseProyeccionVacacionQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `ID`, `USUARIO`, `FECHA_INICIO`, `FECHA_FIN`, `PERIODO`, `DIA_VACACION`, `ESTATUS`, `USUARIO_CREO`, `FECHA_CREA` FROM `proyeccion_vacacion` WHERE `ID` = :p0';
        try {
     ...
     * Example usage:
     * <code>
     * $query->filterByDiaVacacion(true); // WHERE dia_vacacion = true
     * $query->filterByDiaVacacion('yes'); // WHERE dia_vacacion = true
     * </code>
     *
     * @param     boolean|string $diaVacacion The value to use as filter.
    ...
    {
        if (is_string($diaVacacion)) {
            $dia_vacacion = in_array(strtolower($diaVacacion), array('false', 'off', '-', 'no', 'n', '0', '')) ? false : true;
        }
